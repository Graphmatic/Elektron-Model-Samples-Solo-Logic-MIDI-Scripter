...
/*******************************************************NOPS©**********************************************************/
    ...
    /* no needs to modify them here: do that from module GUI then use "save" option from the contextual menu 
       of the module's GUI (the one at the top of the module), and your settings will be recalled even if you
       unload/reload the module or if you load it from another project. */
    ...
    var settingsCCchannel = 1;                // setting of LFO Setup menu of Track 1 on the M:S/M:C =>
                                              ...
                                              // values 64, 65, 66 but don't worry: if you choose another CC, simply 
                                              // sends 0,1,2 values. Values are only remapped if CC.104 is used.
                                              // This setting could be changed from module UI.
    
    var sendSettingsCCtoMS = true;            // Set to true if you want Logic Scripter UI to send to M:S/M:C (or to
    ...
    var reScaleMin = 0;                       // Set reScaleMin and reScaleMax to the desired range. It's usefull if you
                                              ...
                                              // it's not so handly to move the encoder only one step at once. 
                                              // Note that it could sometime produce some weird behaviour when 
                                              // sendSettingsCCtoMS flag is set to true too and encoder is twisted too
                                              // fast.
    ...
    var limitEncoderRange = true;             // with this flag set to true, the M:S/M:C encoder (and so M:S/m:C screen
                                              ...
                                              // controler so it increase MIDI traffic. I suppose it's not a good idea
                                              // to set this flag if you use DIN MIDI of M:S/M:C with 'MIDI THRU' set.

    var listenProgramChange = true;           // Set to false if you don't want to send Mute/Solo states on pattern
                                              // change
...
function HandleMIDI(event)
{
    ...
    if ( ( event instanceof ControlChange ) && ( event.number == settingsCC ) && ( event.channel == settingsCCchannel ) )
    {
        ...
        else if ( ( reScaleSettingsCCvalues === false ) && ( settingsCC === 104 ) ) 
        // the M:S is used --> CC.values must be in the range of 0 to 2 (included).
        {
            modeSwitchingValue -= 64;
        ...
        switch ( modeSwitchingValue ) 
        {
            ...
            case 1:                   // TOGGLING
                soloMode = 1;         // ensure we don't get some résiduel solos from stacked mode
                SoloStateTrigger = SoloStateMemory = 0b000000;
                SetParameter( 0, 1 );
                break;
    ...
    { 
        ...
        /* 
           ...
           will be solo-ed regardless of its previous state (i.e. if the resp. track is Muted before the first 
           stroke, it will be right away un-muted AND solo-ed). It's ensure that track's states are consistent
           with datas stored here.
        */
        ...
        else 
        {
            ...
            /*
              if it's a Solo ON event ( event.value === 1 ), force pressed track to re-un-mute right away (we want
              to transform a mute (off) to a solo (on) )...
...
function ParameterChanged( pParamNumber, pValue ) 
{
    if ( !refreshPatternMenu && !refreshPlayMenu && ( pValue !== lastSavedValues[pParamNumber] ) ) 
    {
        ...
        if ( !nop && ( pParamNumber === 0 ) && sendSettingsCCtoMS ) 
        // Call from UI, must update M:S CC
        {   
            ...
            if ( reScaleSettingsCCvalues === true )
            {
                ...
                // when CC value step down to the range resp. to the lower value, it jump to the
                // lowest value of this range so when it happen we add (reScaleMax / 3)- 1
            ...
            if ( settingsCC === 104 ) 
            {
                // remap value if the "FADE" setting of LFO Setup menu of Track 1 on the M:S/M:C
                settingsCcValue += 64;  
            }
                        ...
                        if ( !refreshPatternMenu && !isPlaying )
                        {
                            ...
                            for ( var o  =  1; o <= bankArray.length; o++ )
                            {
                                for ( var i = 0; i <= 15; i++ )
                                {
                                  ...
                                  var idxx = i + ( (o - 1) * 16 );    
                                  if ( programChangeMemory[ idxx ] !== false )
                                  {
                                    k += "  ";
                                    for ( var loop = 1; loop <= 6; loop++ )
                                    {
